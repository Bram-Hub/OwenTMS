...
class NewTransitionDialog extends JDialog
{
  ...
  public NewTransitionDialog(Edge transition, SortedListModel transitions, String machineType, boolean edit, TransitionsPane transitionpanel)
  {
    ...
    if(edit)
    {
      ...
      if(transition.newChar == TM.NULL)
        newCharText.setText("NULL");
  ...
  void updateTransition()
  {
    ...
    if(newCharText.getText().length() > 1)
      ...
      transition.newChar = newCharText.getText().charAt(0);
  ...
  boolean checkItems()
  {
    ...
    if(machineType.equals("Quadruple Machine") &&
       (!newCharacter.equals(String.valueOf((char)TM.NULL)) &&
       !((String)directionPick.getSelectedItem()).equals("NULL")))
    {
      WarningBox temp = new WarningBox("Error: Not a Quadruple Machine transition", this);
      temp.validate();
      temp.show();
      return false;
    }
    if(machineType.equals("Quintuple Machine") &&
       ...
       ((String)directionPick.getSelectedItem()).equals("NULL")))
    {
      WarningBox temp = new WarningBox("Error: Not a Quintuple Machine transition", this);
      temp.validate();
      temp.show();
      return false;
    }
    if(!edit)
    {
      for(int i = 0; i < transitions.size(); i++)
      {
        ...
        if(n.fromState == transition.fromState)
        {
          ...
          {
            WarningBox temp = new WarningBox("Error: Makes machine non-deterministic", this);
            temp.validate();
            temp.show();
            return false;
          }
...
class NewStateDialog extends JDialog
{
  ...
  void updateState()
  {
    ...
    {
      if(((State)states.elementAt(i)).currentState)
        current = true;
      if(((State)states.elementAt(i)).startState)
        start = true;
    }
    if(!current)
