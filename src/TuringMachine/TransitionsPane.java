...
/**
 * List Model for the TransitionsPane -
 * Maintains list of edges sorted by starting state
 * @author Owen F. Kellett
 * @version 1.0
 */
class SortedListModel extends DefaultListModel
{
  ...
  /**
   * Adds an edge into the list in the appropriate sorted location according
  ...
  public void addSortedElement(Object obj)
  {
    ...
    for(j = 0; j < size(); j++)
    {
      if(((Edge)getElementAt(j)).fromState.stateName.compareTo(((Edge)obj).fromState.stateName) < 0)
        ;
      else if(((Edge)getElementAt(j)).fromState.stateName.compareTo(((Edge)obj).fromState.stateName) == 0)
      {
