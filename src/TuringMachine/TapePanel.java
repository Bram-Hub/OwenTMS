    ...
    implements Runnable, MouseListener
{
  ...
  public TapePanel()
  {
    ...
    tapeFont = new Font("Helvetica", Font.BOLD, 14);
    setSize(600, 50);
  }
  ...
  {
    Dimension layout = new Dimension(size().width, size().height);
    setSize(layout);
    width = layout.width; height = layout.height;
    ypos = height/2 - CELLHEIGHT/2;
    numCells = width/CELLWIDTH - 2;
    xpos = (width - numCells*CELLWIDTH) / 2;
    int[] ltx = {18, 1, 18, 18};
    leftTriXs = ltx;
    int[] rtx = {width-19, width-2, width-19, width-19};
    rightTriXs = rtx;
  ...
  public synchronized void paint(Graphics g)
  {
    ...
    initGraphics();
    offgraphics.setFont(tapeFont);
    offgraphics.drawRect(0, 0, width-1, height-1);
    if (machine.moving == TM.LEFT)
      offgraphics.setColor(Color.yellow);
    else
      offgraphics.setColor(Color.red);
    offgraphics.fillPolygon(leftTriXs, triYs, 4);
    if (machine.moving == TM.RIGHT)
      offgraphics.setColor(Color.yellow);
    else
      offgraphics.setColor(Color.red);
    offgraphics.fillPolygon(rightTriXs, triYs, 4);
    offgraphics.setColor(Color.black);
    offgraphics.drawPolygon(leftTriXs, triYs, 4);
    offgraphics.drawPolygon(rightTriXs, triYs, 4);

    offgraphics.setColor(Color.black);
    int actualNumCells = numCells;
   /* if (!machine.programmed)
    {
   ...
   // else
   // {
      Vector tape = machine.tape;
      int tapePos = machine.tapePos;
           ...
           x+=CELLWIDTH, tpos++)
      {
        ...
        {
          offgraphics.setColor(Color.red);
          offgraphics.drawRect(x, ypos, CELLWIDTH, CELLHEIGHT);
          offgraphics.drawRect(x-1, ypos-1, CELLWIDTH+2, CELLHEIGHT+2);
          offgraphics.setColor(Color.black);
        }
        else
        {
        ...
        if (((Character)tape.elementAt(tpos)).charValue() != ' ')
          offgraphics.drawString("" + ((Character)tape.elementAt(tpos)).charValue(), x+CELLWIDTH/2-4, ypos+CELLHEIGHT*3/4);
      }
  ...
  public void mousePressed(MouseEvent e)
  {
    ...
    if (!execution.isAlive() && !scroller.isAlive())
    {
      if (e.getX() >= leftTriXs[1] && e.getX() <= leftTriXs[0] && e.getY() >= triYs[0] && e.getY() <= triYs[2])
      {
        scrollDir = TM.LEFT;
        machine.scrollTape(scrollDir);
        machine.moving = scrollDir;
        repaint();
        scroller = new Thread(this);
        scroller.start();
      }
      else if (e.getX() >= rightTriXs[0] && e.getX() <= rightTriXs[1] && e.getY() >= triYs[0] && e.getY() <= triYs[2])
      {
        scrollDir = TM.RIGHT;
        machine.scrollTape(scrollDir);
        machine.moving = scrollDir;
...
class TapeCellRenderer extends JLabel implements TableCellRenderer
{
        ...
        int column)
    {
      if(value.toString().equals("-"))
        setIcon(new ImageIcon(TuringMachine.TuringMachineFrame.class.getResource("tapeindex.gif")));
      else
        setIcon(null);
      if (isSelected)
      {
